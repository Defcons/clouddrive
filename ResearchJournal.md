...
| A6 | 2026-08-20: **round-6 hardening** — L2 hourly maintenance ticker (runtime trash/session/upload cleanup), L3 audit-log rotation at 5 MiB + two-generation `GetRecent` (`TestAuditLogRotation`), L6 deleted the dead `UserStore.save()`, L7-part (compose `no-new-privileges`/`cap_drop` + workflow `permissions:{}`), M5 (Syncthing GUI → `127.0.0.1`), M10 (bounded `ReadTimeout` 15m, `WriteTimeout` left off for streaming). | Safe items auto-deployed; M6 (non-root uid/gid 1000) parked on its own branch pending a one-time host `chown -R 1000:1000 /data`. | **Deferred with reason:** M8 quota-accounting (dotdirs are globally-keyed → needs per-user attribution + a free-space semantics call); L7 base-image DIGEST pinning (no registry/docker in the work env); L8 PDF-iframe sandbox (already nosniff-mitigated, sandbox would risk breaking the viewer). | KEEP |
