...
func newTestStore(t *testing.T, password string) *UserStore {
	...
	cfg := models.UsersConfig{Users: []models.User{
		{Username: "bob", Password: hash, HomeFolder: "/", Role: "admin"},
	}}
...
func TestAuthenticate(t *testing.T) {
	...
	t.Run("correct credentials succeed", func(t *testing.T) {
		u, err := store.Authenticate("bob", "correct horse battery")
		if err != nil || u == nil || u.Username != "bob" {
			t.Fatalf("expected success, got user=%v err=%v", u, err)
		}
	})

	t.Run("wrong password fails", func(t *testing.T) {
		if _, err := store.Authenticate("bob", "wrong"); err == nil {
	...
	t.Run("returned user is a snapshot copy", func(t *testing.T) {
		u, _ := store.Authenticate("bob", "correct horse battery")
		u.Role = "tampered"
		again, _ := store.Authenticate("bob", "correct horse battery")
		if again.Role != "admin" {
