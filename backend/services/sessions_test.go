...
func TestSessionStoreLifecycle(t *testing.T) {
	...
	id := s.Create("alice", "Firefox", "1.2.3.4", 1000)
	...
	if got := s.List("alice"); len(got) != 1 || got[0].ID != id {
		t.Fatalf("List(alice) = %v", got)
	}

	// Another non-admin user cannot revoke alice's session.
	if s.Revoke(id, "bob", "user") {
	...
	// Owner can revoke.
	if !s.Revoke(id, "alice", "user") || s.IsValid(id) {
...
func TestSessionRevokeAllAndAdmin(t *testing.T) {
	...
	if s.IsValid(a) || s.IsValid(b) {
		t.Error("RevokeAllForUser should clear alice's sessions")
	}
	if !s.IsValid(other) {
