...
func TestMoveToTrashAndRestore(t *testing.T) {
	...
	if err := store.MoveToTrash(orig, "/doc.txt", "bob"); err != nil {
	...
	items := store.List("bob", "user")
	if len(items) != 1 {
		t.Fatalf("want 1 trashed item, got %d", len(items))
	}

	if err := store.Restore(items[0].ID, "bob", "user"); err != nil {
...
func TestRestoreDeniesTraversal(t *testing.T) {
	...
	err := store.Restore("evil", "bob", "admin")
	if err == nil {
...
func TestRestoreDoesNotClobber(t *testing.T) {
	...
	if err := store.MoveToTrash(orig, "/report.txt", "bob"); err != nil {
		t.Fatal(err)
	}
	// A new file now occupies the original location.
	writeFile(t, orig, "new version")

	items := store.List("bob", "admin")
	if err := store.Restore(items[0].ID, "bob", "admin"); err != nil {
		t.Fatalf("Restore: %v", err)
	}
...
func TestMoveToTrashUniqueIDs(t *testing.T) {
	...
	for _, content := range []string{"first", "second"} {
		...
		if err := store.MoveToTrash(p, "/same.txt", "bob"); err != nil {
			t.Fatal(err)
		}
	}
...
func TestRestorePermissionDenied(t *testing.T) {
	...
	orig := filepath.Join(root, "Alice", "private.txt")
	writeFile(t, orig, "secret")
	if err := store.MoveToTrash(orig, "/Alice/private.txt", "alice"); err != nil {
		t.Fatal(err)
	}
	items := store.List("alice", "admin")

	// A different non-admin user must not be able to restore alice's item.
	if err := store.Restore(items[0].ID, "bob", "user"); err == nil {
