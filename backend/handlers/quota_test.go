...
func uploadAs(t *testing.T, h *FileHandler, target string, body *bytes.Buffer, contentType string) *httptest.ResponseRecorder {
	...
	req.Header.Set("Authorization", "Bearer "+sessionToken(t, "alice", "user", "/Alice"))
...
func TestUploadEnforcesQuota(t *testing.T) {
	...
	if rec := uploadAs(t, h, "/Alice", body, ct); rec.Code != http.StatusInsufficientStorage {
	...
	if rec := uploadAs(t, h, "/Alice", body2, ct2); rec.Code != http.StatusOK {
...
func TestUploadNoQuotaWhenUnset(t *testing.T) {
	...
	if rec := uploadAs(t, h, "/Alice", body, ct); rec.Code != http.StatusOK {
