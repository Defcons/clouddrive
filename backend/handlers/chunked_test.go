...
func TestChunkedUploadAssembles(t *testing.T) {
	...
	tok := sessionToken(t, "alice", "user", "/Alice")
	uid := "upload-abc123"
	parts := []string{"Hello, ", "chunked ", "world!"}

	for i, p := range parts {
		target := fmt.Sprintf("/api/files/upload/chunk?uploadId=%s&index=%d&path=/Alice", uid, i)
		if rec := serve(h.UploadChunk, http.MethodPost, target, p, tok); rec.Code != http.StatusOK {
			t.Fatalf("chunk %d: got %d", i, rec.Code)
		}
	}

	body := fmt.Sprintf(`{"uploadId":%q,"name":"greeting.txt","path":"/Alice","total":3}`, uid)
	...
	got, err := os.ReadFile(filepath.Join(root, "Alice", "greeting.txt"))
	if err != nil {
...
func TestChunkedUploadMissingChunkFails(t *testing.T) {
	root := t.TempDir()
	_ = os.MkdirAll(filepath.Join(root, "Alice"), 0755)
	h := NewFileHandler(root, nil, nil, nil, nil, nil)
	tok := sessionToken(t, "alice", "user", "/Alice")
	uid := "upload-gap999"

	// Send only chunk 0 but claim total 2.
	serve(h.UploadChunk, http.MethodPost, "/api/files/upload/chunk?uploadId="+uid+"&index=0&path=/Alice", "x", tok)
	body := fmt.Sprintf(`{"uploadId":%q,"name":"f.txt","path":"/Alice","total":2}`, uid)
...
func TestChunkedUploadOwnershipEnforced(t *testing.T) {
	...
	// alice starts the upload...
	serve(h.UploadChunk, http.MethodPost, "/api/files/upload/chunk?uploadId="+uid+"&index=0&path=/Alice", "x", sessionToken(t, "alice", "user", "/Alice"))
	// ...bob (admin, so checkAccess passes) must not be able to add to it.
	rec := serve(h.UploadChunk, http.MethodPost, "/api/files/upload/chunk?uploadId="+uid+"&index=1&path=/Alice", "y", sessionToken(t, "bob", "admin", "/"))
